...
export const ArrowCursor = ({ color = "#F2F5F8", size = 20 }: ArrowCursorProps) => {
	...
	useEffect(() => {
		...
		// Handle click scale and ripple animations
		const handlePointerDown = () => {
			if (scaleRef.current) {
				scaleRef.current.style.transform = "scale(0.75)";
			}
		...
		// Force hide system cursor globally and inject keyframes
		...
		style.innerHTML = `
			* {
			...
			.cursor-ripple-active {
				animation: cursor-ripple 0.4s cubic-bezier(0.16, 1, 0.3, 1) forwards;
			}
		`;
