...
export const FluxBackground: React.FC<EnergyFlowBackgroundProps> = ({
    ...
    useEffect(() => {
        ...
        const fsSource = \\\`
            ...
            uniform float u_intensity; uniform float u_scale; uniform float u_opacity;
            uniform float u_vignetteIntensity; varying vec2 vUv;
            ...
            float snoise(vec2 v){
              ...
              vec3 a0 = x - floor(x + 0.5); vec3 g = a0 * vec3(x0.x,x12.xz) + h * vec3(x0.y,x12.yw);
              vec3 l = 1.79284291400159 - 0.85373472095314 * ( g*g + h*h );
              vec3 normalize_v = g * l; return 130.0 * dot(m, normalize_v);
            }
        ...
        let frame: number; const startTime = performance.now();
        const loop = () => {
            ...
            gl.uniform2f(resLoc, canvas.width, canvas.height); gl.uniform2f(mouseLoc, mousePos.current.x, mousePos.current.y);
            gl.uniform3f(colorALoc, rgbA[0], rgbA[1], rgbA[2]); gl.uniform3f(colorBLoc, rgbB[0], rgbB[1], rgbB[2]);
            gl.uniform3f(colorCLoc, rgbC[0], rgbC[1], rgbC[2]); gl.uniform1f(intensityLoc, intensity);
            gl.uniform1f(scaleLoc, scale); gl.uniform1f(opacityLoc, opacity);
            gl.uniform1f(vignetteLoc, vignetteIntensity); gl.drawArrays(gl.TRIANGLES, 0, 6);
    ...
    return (
        <div className=\\\`relative isolate w-full h-full overflow-hidden bg-black \\\${className}\\\`>
            <canvas ref={canvasRef} className="absolute inset-0 w-full h-full block" style={{ filter: blurAmount > 0 ? \\\`blur(\\\${blurAmount}px)\\\` : "none" }} />
            {children && (
