...
import SearchDialog from "../buttons/SearchDialog";
...
export default function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const currentIndex = React.useMemo(
    () => registryEntries.findIndex((item) => item.id === activeItem),
    [activeItem, registryEntries],
  );
  const prevItem = currentIndex > 0 ? registryEntries[currentIndex - 1] : null;
  const nextItem =
    currentIndex < registryEntries.length - 1 && currentIndex !== -1
      ? registryEntries[currentIndex + 1]
      : null;
  ...
  const [activeMainNav, setActiveMainNav] =
    React.useState<string>("components");
  ...
  return (
    <div className="flex flex-col h-[100dvh] overflow-hidden font-sans bg-background text-foreground transition-colors duration-500">
      ...
      <div className="flex flex-1 overflow-hidden w-full max-w-[100vw] justify-between md:px-2 md:py-3 relative">
        ...
        {/* Left Sidebar */}
        <aside
          className={`fixed inset-y-0 left-0 z-[70] w-[280px] max-w-[90vw] bg-rb-neutral-2 border-r border-white/5 md:bg-transparent md:border-none md:relative md:w-auto h-full flex flex-col transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen
              ? "translate-x-0"
        ...
        >
          ...
          <div className="flex md:hidden items-center justify-between pt-6 px-[18px] border-b border-white/5 bg-rb-neutral-2 flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <span className="text-rb-accent-2 font-semibold text-lg tracking-tight ml-2">
                Navigation
              </span>
            </div>
            <button
